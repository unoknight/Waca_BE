...
module.exports = {
    ...
    getNguoiGioiThieu: async (email, callback) => {
        ...
        // Tổng hoa hồng giao dịch mọi tháng
        await new Promise((resolve, reject) => {
            db.query(
                `select 
        ...
        let TSNGD = 0,
            TSDL = 0,
            TSNGDTN = 0, // tổng số nhà giao dịch tháng này
            TSNGDTT = 0, // tổng nhà giao dịch tháng trước
            TSDLM = 0, // tổng đại lý mới tháng này
            TSDLTT = 0, // tổng đại lý mới tháng trước
            tslgdCD1_local = 0, // tổng giao dịch cấp dưới tháng này
            tslgdCD2_local = 0,
            tslgdCD3_local = 0,
            tslgdCD4_local = 0;
        ...
        obj.tsngd = TSNGD;
        obj.tsdl = TSDL;
        obj.tslgdmtn = TSNGDTN; // tổng số nhà giao dịch tháng này
        obj.tslgdmtt = TSNGDTT; // tổng số nhà giao dịch tháng trước
        obj.tsdlmtn = TSDLM; // tổng đại lý mới tháng này
        obj.tsdlmtt = TSDLTT; // tổng đại lý mới tháng trước
        obj.tslgdCD1 = tslgdCD1_local; // tổng giao dịch cấp dưới tháng này
        obj.tslgdCD2 = tslgdCD2_local; // tổng giao dịch cấp dưới tháng trước
        obj.tslgdCD3 = tslgdCD3_local; // tổng giao dịch cấp dưới tháng trước nữa
        obj.tslgdCD4 = tslgdCD4_local; // tổng giao dịch cấp dưới tháng trước trước nữa
        ...
        let listAgentF1 = await new Promise((resolve, reject) => {
            // tổng số đại lý ( đã mua vip ) của bản thân
            // AND vip_user = ?
            db.query(
        ...
        await new Promise((resolve, reject) => {
            ...
            let min = 0;
            let max = listAgentF1.length;
            if (max == 0) resolve();
            let totalDLVip = obj.tsdlisMe
            listAgentF1.forEach(function (item) {
                ...
                db.query(
                    `SELECT SUM(bet_history.amount_bet) AS hhttisMe FROM bet_history
                    WHERE bet_history.email = ? AND bet_history.type_account = ? AND bet_history.marketing = ? AND WEEKOFYEAR(bet_history.created_at) = WEEKOFYEAR(NOW())`,
                    [
                        ...
                        if (!!results[0].hhttisMe) {
                            min++;
                            obj.hhttisMe += Number.parseFloat(results[0].hhttisMe);
                        } else {
                            resolve();
                        }
                    })
                ...
                db.query(
                    `SELECT SUM(copy_trade_history.value) AS hhttisMe FROM copy_trade_history
                        JOIN users ON users.email = copy_trade_history.email AND users.marketing = 0
                        WHERE copy_trade_history.email = ? AND copy_trade_history.acc_type = ? AND WEEKOFYEAR(copy_trade_history.created_at) = WEEKOFYEAR(NOW())`,
                    [
                        item.email, // mã của f1 đại lý
                        1,
                        //refForMe,
